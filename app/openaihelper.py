...
class OpenAIHelper:
    ...
    def send_message(self, messages):
        ...
        if ("[ClaudeJB]" in message):
            ...
            time.sleep(1)
            self.bot.send_message("Continue.")
        else:
            self.bot.send_message(message)
    ...
    def generate_completions(self, messages):
        ...
        while checks < self.maxchecks:
            if not self.bot.is_generating() and self.bot.get_latest_message() != "" and self.bot.get_latest_message() != self.lastmessage:
                break
            checks += 1
            time.sleep(1)
        self.lastmessage = self.bot.get_latest_message()
